...
def get_ar6_data(ar6_dir,confidence,scenario,years,quantiles,tg_id=None,coords=None,print_info=True,baseline=None,surge=None):
    ...
    for y in np.atleast_1d(years):
        idx_year = np.atleast_1d(np.argwhere(AR6_years == y).squeeze())[0]
        for q in np.atleast_1d(quantiles):
...
def main():
    ...
    parser.add_argument('--quantile',help='Quantile to use?',default=0.5,type=float)
    parser.add_argument('--years',help='Years to use?',default=None,nargs='*')
    parser.add_argument('--surge',help='Surge to use?',default=0.0,type=float)
